package cn.zhxs.core.query.parse;
...
import cn.zhxs.core.query.data.Condition;
import cn.zhxs.core.query.data.Condition.Filter;
import cn.zhxs.core.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.zhxs.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.zhxs.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.zhxs.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.zhxs.core.query.data.Sort.Direction.ASC) {
